...
export class SbomCirclePacking {
  ...
  update(data: GraphData) {
    ...
    const pack = d3.pack<Node>().size([diameter, diameter]).padding(2);
    pack(root as any);
  ...
  private render(root: d3.HierarchyNode<Node>) {
    ...
    const circle = g
      ...
      .data(nodes)
      .join("circle")
      .attr("cx", (d: any) => d.x - root.x)
      .attr("cy", (d: any) => d.y - root.y)
      .attr("r", (d: any) => d.r)
      .attr("fill", (d: any) => {
    ...
    const label = g
      ...
      .data(nodes)
      .join("text")
      .attr("x", (d: any) => d.x - root.x)
      .attr("y", (d: any) => d.y - root.y)
      .attr("dy", "0.3em")
      .attr("text-anchor", "middle")
      .attr("fill", "#fff")
      .style("font-size", (d: any) => `${Math.max(8, 10 - d.depth)}px`)
      .text((d: any) => {
        if (d.depth === 1)
